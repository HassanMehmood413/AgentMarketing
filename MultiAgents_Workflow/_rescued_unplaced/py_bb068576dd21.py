from prompt.section_writer import section_writer_instructions
...
def write_section(state:ResearchState):
    ...
    print(f"[write_section] Analyst: {analyst.name if hasattr(analyst, 'name') else 'Unknown'}", file=sys.stderr)
    ...
    if not context or len(str(context)) == 0:
        ...
        return {'sections': [f"ERROR: No context available for {analyst.name if hasattr(analyst, 'name') else 'Unknown'}"]}
    ...
    section_content = section.content if section else f"ERROR: No response from OpenAI for {analyst.name if hasattr(analyst, 'name') else 'Unknown'}"
